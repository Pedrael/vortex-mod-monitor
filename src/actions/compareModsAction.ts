...
export default function createCompareModsAction(
  ...
  return async () => {
    try {
      ...
      const outputDir = path.join(appDataPath, "mod-auditor", "diffs");
      ...
      console.log(
        `[Vortex Mod Auditor] Diff generated | referenceOnly=${diff.summary.onlyInReference} | currentOnly=${diff.summary.onlyInCurrent} | changed=${diff.summary.changed}`,
      );
      ...
      console.error("[Vortex Mod Auditor] Compare failed:", error);
    }
  };
}
