...
- **Diff engine**: included in `COMPARE_FIELDS`, so two snapshots with the same `compareKey` but different `archiveSha256` produce a `ModFieldDifference` entry. See [`COMPARE_MODS.md`](COMPARE_MODS.md).
