...
5. Show an `"activity"` notification with id `"vortex-mod-monitor:hashing"` and message `"Hashing N mod archives…"`.
...
9. Compute four diagnostic counts:
...
10. Compute the output directory: `<appData>/mod-monitor/exports/`.
11. Call `exportModsToJsonFile({ mods, gameId, profileId, outputDir, deploymentManifests })`:
    1. `mkdir -p` the output directory.
    2. Construct filename `vortex-mods-<gameId>-<profileId>-<unixMillis>.json`.
    3. Write a JSON document containing `exportedAt` (ISO 8601), `gameId`, `profileId`, `count`, `mods`, and (when present) `deploymentManifests` — pretty-printed with 2-space indent. The `deploymentManifests` field is omitted from the JSON entirely when undefined, preserving older-format compatibility.
12. Log a single line to console with the diagnostic counts.
13. Show a `"success"` notification with two action buttons: "Open Export" (opens the file via `start ""`) and "Open Folder" (opens the directory).
...
- **Path**: `<appData>\mod-monitor\exports\vortex-mods-<gameId>-<profileId>-<unixMillis>.json`
- **Contents**: see [`DATA_FORMATS.md`](../DATA_FORMATS.md#1-mods-snapshot--vortex-mods-gameid-profileid-tsjson).
- **Encoding**: UTF-8.
...
| `success` | On completion | `Exported N mods | FOMOD: X | Hashed: Y/N | Deployed files: D` |
...
One success line:
```
[Vortex Mod Monitor] Exported N mods | game=<gameId> | profile=<profileId> | fomod=X | detailed=Y | hashed=Z/N | deployedFiles=D across M modtype(s)
```

Or one error line:
```
[Vortex Mod Monitor] Export failed: <Error stack>
```
...
- **INVARIANT**: The file name format `vortex-mods-<gameId>-<profileId>-<unixMillis>.json` is parsed by no current code — but expect future versions of the compare action to glob this pattern for "recent snapshots" pickers.
...
- Output schema: [`DATA_FORMATS.md`](../DATA_FORMATS.md#1-mods-snapshot--vortex-mods-gameid-profileid-tsjson)
